 ...
 *
 */
public class AbletonSceneLauncherPage implements Page, Serializable {
				...
				if (x == 0) {
					...
					if (y < (this.monome.sizeY - this.numEnabledRows)) {
						...
						if (y == this.monome.sizeY - 2 && Main.main.configuration.abletonState.getSelectedScene() >= 0) {
							...
							this.launchScene(Main.main.configuration.abletonState.getSelectedScene() - 2);
						} 
						// next scene	
							...
							// pages starts at 0.  i couldn't get liveosc to send me a packet with
							// an argument equal to (int) 0 (wtf?) so i had to start at 1.
							this.launchScene(Main.main.configuration.abletonState.getSelectedScene());
						}
					}
				}
	...
	private void launchScene(int scene_num) {
		Main.main.configuration.getAbletonControl().launchScene(scene_num);
	}
	...
	public void handleTick() {
		...
		for (int y = 0; y < this.monome.sizeY - numEnabledRows; y++) {
			if (Main.main.configuration.abletonState.getSelectedScene() == (1 + y + (this.clipOffset * (this.monome.sizeY - this.numEnabledRows)))) {
				if (tickNum % 24 == 0) {
	...
	public void redrawDevice() {
		for (int scene = 0; scene < this.monome.sizeY; scene++) {
			...
			if (scene_num == Main.main.configuration.abletonState.getSelectedScene()) {
		...
		if (this.monome.curPage == this.index) {
			int widthOffset = this.trackOffset * (this.monome.sizeX - 2);
			int clipOffset = (this.clipOffset * (this.monome.sizeY - this.numEnabledRows));
			int width = this.monome.sizeX - 2;
